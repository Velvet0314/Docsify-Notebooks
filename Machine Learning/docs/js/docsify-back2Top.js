...
function docsifyBackTopFn(config) {
    ...
    window.addEventListener('scroll', function () {
        if (window.scrollY > 0) {
            div.style.opacity = 1;
            div.style.pointerEvents = 'auto';  // 允许鼠标事件
        } 
        else {
        div.style.opacity = 0;
        div.style.pointerEvents = 'none';  // 禁止鼠标事件
        }
    });
}
